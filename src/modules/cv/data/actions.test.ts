...
import type { Cv, PersonalInfo, SectionLayout, Skill } from "./mappers";
...
describe("cv actions", () => {
  ...
  describe("updateCvLayoutAction", () => {
    ...
    it("returns the saved layout on success", async () => {
      mutations.updateCvLayout.mockResolvedValueOnce(layout);

      const result = await updateCvLayoutAction("cv_001", {
        mainOrder: ["summary", "experience"],
        sideOrder: ["skills"],
        hidden: ["certifications"],
        titles: { experience: "Work History" },
      ...
      expect(mutations.updateCvLayout).toHaveBeenCalledWith(
        "cv_001",
    ...
    it("maps an HttpError to a failure result", async () => {
      mutations.updateCvLayout.mockRejectedValueOnce(
        new HttpError(404, "Not Found", "CV not found")
      );
